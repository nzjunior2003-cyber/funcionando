...
const SAFE_BOTTOM_MARGIN = 35; // Zona de segurança blindada para não esmagar o rodapé
...
export const generateOrcamentoLicitacaoPdf = (doc: jsPDF, data: OrcamentoData) => {
    ...
    s1Body.push([
        { 
            content: 'O QUE SERÁ\nPESQUISADO?', 
            rowSpan: data.itemGroups.length + 1, 
            styles: { valign: 'middle', halign: 'center', fillColor: [255,255,255], fontStyle: 'bold', textColor: 0 } 
        },
    ...
    autoTable(doc, { startY: y, body: s4b, theme: 'grid', styles: { fontSize: 8, valign: 'middle', lineColor: 0, lineWidth: 0.1 }, columnStyles: { 0: { cellWidth: 70 }, 1: { cellWidth: 45 } }, margin: { left: MARGIN_LEFT, right: MARGIN_RIGHT, bottom: SAFE_BOTTOM_MARGIN } });
    y = (doc as any).lastAutoTable.finalY + 8;
    ...
    autoTable(doc, { startY: y, head: [['Item', 'Preços Encontrados']], body: s6b, theme: 'grid', headStyles: { fillColor: YELLOW, textColor: 0, halign: 'center' }, styles: { fontSize: 8, lineColor: 0, lineWidth: 0.1, halign: 'center' }, margin: { left: MARGIN_LEFT, right: MARGIN_RIGHT, bottom: SAFE_BOTTOM_MARGIN } });
    ...
    // Final Table (Com Lógica Avançada de Quantidades e Cotas)
    addPage(40);
    ...
    const fb: any[] = [];
    data.itemGroups.forEach(g => {
        // Calcula o total da linha de forma absoluta, independente de cotas com erro
        const totalLinha = g.estimativaUnitaria * (g.quantidadeTotal || 0); 
        total += totalLinha;
        
        // Verifica se as cotas realmente possuem quantidades preenchidas (maior que 0)
        ...
        if (cotasValidas && cotasValidas.length > 0) {
            cotasValidas.forEach((c, i) => {
                fb.push([
                    ...
                    formatValue(g.estimativaUnitaria, g.tipoValor), 
                    c.quantidade, 
                    formatValue(c.quantidade * g.estimativaUnitaria, g.tipoValor)
                ]);
            });
        } else {
            // Fallback: Se não tem cota, ou a cota veio com quantidade 0, imprime o item unificado
            let cotaLabel = 'AMPLA'; 
            if (g.cotas && g.cotas.length > 0) {
                // Se existe uma cota, mas ela estava zerada, puxa a etiqueta dela
