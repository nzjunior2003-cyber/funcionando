...
const parseValueLocal = (value: string | number | undefined, tipoValor: 'moeda' | 'percentual' = 'moeda'): number | null => {
    ...
    let strVal = String(value).trim().replace(/[R$%\s]/g, '').replace(/[^\d,.-]/g, '');
    if (strVal.includes(',')) {
        strVal = strVal.replace(/\./g, '').replace(',', '.');
    } else if (strVal.includes('.')) {
        const parts = strVal.split('.');
        if (parts[parts.length - 1].length === 3) strVal = strVal.replace(/\./g, '');
    }
    const num = parseFloat(strVal);
    return isNaN(num) ? null : num;
};

const calculateEstimateLocal = (prices: (string | undefined)[], methodology: string, tipoValor: 'moeda' | 'percentual'): number => {
    const validPrices = prices.map(p => parseValueLocal(p, tipoValor)).filter((p): p is number => p !== null && (tipoValor === 'percentual' || p > 0));
...
const nomesFontesCurto: Record<string, string> = {
    ...
    contratacaoSimilar: 'Contratação Similar', direta: 'Fornecedor', preco_ata_srp: 'Ata SRP'
};

export const generateOrcamentoAdesaoAtaPdf = (doc: jsPDF, data: OrcamentoData) => {
    let y = MARGIN_TOP;
    setDefaultFont(doc);
    ...
    const s1Body: any[] = data.itemGroups.map((g) => {
        return [
            ...
            { content: g.descricao, styles: { halign: 'justify' } }, g.codigoSimas || '-', g.unidade, g.quantidadeTotal
        ];
    });
    autoTable(doc, {
    ...
    data.itemGroups.forEach(g => {
        ...
        qcb.push([
            ...
            { content: g.descricao, styles: { halign: 'justify' } },
            { content: valMercado > 0 ? formatValue(valMercado, g.tipoValor) : '-', styles: { halign: 'center' } },
            { content: valAta > 0 ? formatValue(valAta, g.tipoValor) : '-', styles: { halign: 'center' } },
    ...
    autoTable(doc, {
        startY: y,
        head: [['Item', 'Descrição', `Valor de Mercado (${metLabel})`, 'Preço da Ata', 'Preço Adotado']],
        body: qcb,
        theme: 'grid',
        ...
        columnStyles: { 0: { cellWidth: 12 }, 2: { cellWidth: 35 }, 3: { cellWidth: 25 }, 4: { cellWidth: 25 } },
        margin: { left: MARGIN_LEFT, right: MARGIN_RIGHT, bottom: SAFE_BOTTOM_MARGIN },
        rowPageBreak: 'avoid',
        willDrawCell: justifyWillDrawCell,
        didDrawCell: justifyDrawCell
