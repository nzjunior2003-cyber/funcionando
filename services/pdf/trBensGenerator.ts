...
export const generateTrBensPdf = (doc: jsPDF, data: TrBensData) => {
    ...
    const advancedDidDrawCell = (hookData: any) => {
        if (hookData.section === 'body') {
            ...
            if (Array.isArray(styles.textColor)) doc.setTextColor(styles.textColor[0], styles.textColor[1], styles.textColor[2]);
            else doc.setTextColor(styles.textColor as any);
            lines.forEach((lineText: string, idx: number) => {
                ...
                if (styles.halign === 'justify') {
                    ...
                    if (isLastLine || isShortLine) doc.text(lineText, textX, textY, { align: 'left', baseline: 'middle' } as any);
                    else doc.text([lineText, ''], textX, textY, { align: 'justify', maxWidth: maxWidth, baseline: 'middle' } as any);
                } else {
                    let finalX = textX;
    ...
    if (hasLote) {
        data.itens.forEach(it => {
            if (it.loteId) lotesTotal[it.loteId] = (lotesTotal[it.loteId] || 0) + ((it.quantidade || 0) * (it.valorUnitario || 0));
        });
    }

    const t1Head: any[] = [];
    t1Head.push([{ content: '1. O QUE SERÁ CONTRATADO?
...
Concorrência';
    ...
    t1Body.push([{ content: 'VALOR GLOBAL ESTIMADO', colSpan: hasLote ? 6 : 5, styles: { fontStyle: 'bold', halign: 'right', valign: 'middle', fillColor: colorGrayLabel } }, { content: formatCurrency(totalGlobal), colSpan: 3, styles: { fontStyle: 'bold', halign: 'right', valign: 'middle', fillColor: colorGrayLabel } }]);
    ...
    let isZebra = false;
    const pushHeader = (title: string) => { t2Body.push([{ content: title, colSpan: 2, styles: { fillColor: colorBlueHeader, textColor: 255, halign: 'center', fontStyle: 'bold' } }]); isZebra = false; };
    const pushRow = (q: string, a: string, justificado: boolean = false) => { const bg = isZebra ? colorGrayLabel : colorWhiteLabel; isZebra = !isZebra; t2Body.push([{ content: q, styles: { fillColor: bg, fontStyle: 'bold', valign: 'middle' } }, { content: a, styles: { fillColor: bg, valign: 'middle', halign: justificado ? 'justify' : 'left' } }]); };
    const pushFullRow = (content: string, justificado: boolean = false) => { const bg = isZebra ? colorGrayLabel : colorWhiteLabel; isZebra = !isZebra; t2Body.push([{ content: content, colSpan: 2, styles: { fillColor: bg, valign: 'middle', halign: justificado ? 'justify' : 'left' } }]); };
...
(art. 6°, XXIII, h, da Lei Federal nº 14.133/21)');
    ...
    pushRow('6.1. FORMA DE CONTRATAÇÃO', [`${radio(fC.includes('inexigibilidade'))} Inexigibilidade (Art. 74, Inciso ${data.inexigibilidadeInciso || '...'})`, `${radio(fC.includes('dispensa_valor'))} Dispensa por Valor (Art. 75, II)`, `${radio(fC.includes('dispensa_art75'))} Dispensa (Art. 75, Inciso ${data.dispensaInciso || '...'})`, `${radio(fC.includes('pregao'))} Pregão eletrônico`, `${radio(fC.includes('pregao_rp'))} Pregão para Registro de Preços`].join('
...
${radio(data.habilitacaoTecnicaExigida === 'nao')} Não.`, true);
    ...
    const qualifTecText = qualifTec.length === 0 ? 'Conforme Edital.' : qualifTec.map(opt => { const baseText = `${mapQualificacaoTecnica[opt] || opt}`; const just = data.qualificacoesTecnicasJustificativas?.[opt]; return just ? `${baseText}
  Justificativa: ${just}` : baseText; }).join('
...
(art. 18, § 1°, VI, da Lei Federal nº 14.133/21)');
    ...
    autoTable(doc, {
        ...
        body: t2Body,
        theme: 'grid',
        styles: { fontSize: 8, lineColor: [0,0,0], lineWidth: 0.1, textColor: 0, overflow: 'linebreak', cellPadding: { top: 1.2, right: 3, bottom: 1.2, left: 1.2 } },
    ...
    if (finalY > PAGE_HEIGHT - 65) { doc.addPage(); finalY = MARGIN_TOP + 10; }
    ...
    doc.setFontSize(10);
    const drawNameWithBoldGuerra = (nomeCompleto: string, nomeGuerra: string, cargo: string, x: number, y: number) => {
        ...
        parts.forEach((p, i) => { doc.setFont('helvetica', p.bold ? 'bold' : 'normal'); doc.text(p.text, startX, y); startX += widths[i]; });
    };

    drawNameWithBoldGuerra(data.nome || '', data.nomeGuerra || '', data.cargo || '', sigX, finalY + 5);
    if (data.funcao) { doc.setFont('helvetica', 'normal'); doc.text(data.funcao, sigX, finalY + 10, { align: 'center' }); }

    const totalPages = (doc as any).internal.getNumberOfPages();
    for (let i = 1; i <= totalPages; i++) {
        doc.setPage(i);
        if (i === totalPages) drawInstitutionalFooter(doc, data.setor || '', i, totalPages);
        else { doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.text(`Página ${i} de ${totalPages}`, PAGE_WIDTH - R_MARGIN, PAGE_HEIGHT - 10, { align: 'right' }); }
    }
};
