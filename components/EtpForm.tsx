...
import { cargoOptions, inputClasses, parseCurrency, formatCurrencyInput, Field, SectionWithHelp as Section } from './formCommon';
...
export const EtpForm: React.FC<EtpFormProps> = ({ data, setData }) => {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setData(prev => {
        ...
        return {
            ...prev,
            [field]: checked ? [...currentArr, value] : currentArr.filter(v => v !== value)
        };
    });
  };
  ...
  const handlePriceInputChange = (id: string, value: string) => {
      // Permite apenas números e uma vírgula durante a digitação
    ...
    <div className="space-y-6">
      <Section title="Identificação do Documento">
        <div className="grid md:grid-cols-4 gap-6">
          <Field label="Setor" required><input type="text" name="setor" value={data.setor || ''} onChange={handleChange} required className={inputClasses} placeholder="Ex: DAL, BMRS..." /></Field>
      ...
      <Section title="1 – DESCRIÇÃO DA NECESSIDADE" instruction="Descrever a necessidade da contratação do bem ou serviço, a partir da identificação do que o órgão realmente precisa, considerando o problema exposto no documento de formalização de demanda.">
        <Field label="1.1. Qual a necessidade a ser atendida?" required>
          <div className="relative">
            <textarea name="necessidade" value={data.necessidade} onChange={handleChange} required className={`${inputClasses} h-24 pr-10`} />
            <AiAssistant fieldName="Necessidade a ser Atendida" onGeneratedText={(text) => setData(prev => ({ ...prev, necessidade: text }))} />
          </div>
        </Field>
      </Section>
      ...
      <Section title="2 – LEVANTAMENTO DE MERCADO" instruction="Registrar os meios utilizados na pesquisa para levantar as soluções possíveis para o problema. Na justificativa técnica e econômica para a escolha da solução, deve-se informar o levantamento de mercado realizado com a análise das soluções possíveis, concluindo com a indicação daquela considerada mais viável para atender a necessidade da administração pública. Além disso, caso seja possível adquirir ou alugar o bem, deve-se traçar comparativo de custos e benefícios de cada opção, com indicação da alternativa mais vantajosa. Após o levantamento, caso se verifique que a quantidade de fornecedores é restrita, deve-se refletir se os requisitos que limitaram a competição são indispensáveis para a boa execução do contrato ou segurança da administração pública. Se não forem, devem ser flexibilizados.">
        ...
        <Field label="2.2. Justificativa técnica e econômica para a escolha da melhor solução" required>
            <div className="relative">
                <textarea name="justificativaTecnica" value={data.justificativaTecnica} onChange={handleChange} required className={`${inputClasses} h-24 pr-10`} />
                <AiAssistant fieldName="Justificativa Técnica e Econômica" onGeneratedText={(text) => setData(prev => ({ ...prev, justificativaTecnica: text }))} />
            </div>
        </Field>
      ...
      <Section title="4 – DESCRIÇÃO DA SOLUÇÃO" instruction="Indicar o conjunto de todos os elementos (bens, serviços e outros) necessários para, de forma integrada, gerar os resultados que atendam à necessidade que gerou a contratação, bem como informar necessidades relacionadas à manutenção, assistência técnica e garantia, quando for o caso.">
         <Field label="4.1. O que será contratado?">
            <div className="relative">
                <textarea name="solucaoContratacao" value={data.solucaoContratacao} onChange={handleChange} className={`${inputClasses} h-24 pr-10`}/>
                <AiAssistant fieldName="Solução a ser Contratada" onGeneratedText={(text) => setData(prev => ({ ...prev, solucaoContratacao: text }))} />
            </div>
         </Field>
         <Field label="4.2. Qual o prazo da garantia contratual?">
            ...
            {data.garantiaContratual === 'outro' && <div className="flex items-center gap-2 mt-2 p-2 border rounded-md bg-white dark:bg-gray-800 dark:border-gray-600"><input type="number" min="0" name="garantiaOutroNum" value={data.garantiaOutroNum} onChange={handleChange} className={`${inputClasses} w-1/3`} placeholder="Nº"/><select name="garantiaOutroTipo" value={data.garantiaOutroTipo} onChange={handleChange} className={`${inputClasses} w-2/3`}><option value="dias">dias</option><option value="meses">meses</option><option value="anos">anos</option></select></div>}
         </Field>
         <Field label="4.3. Há necessidade de assistência técnica?">
            ...
            {data.assistenciaTecnica === 'sim' && (
                <div className="mt-2">
                    ...
                    <textarea
                        name="justificativaAssistenciaTecnica"
                        value={data.justificativaAssistenciaTecnica || ''}
                        onChange={handleChange}
                        className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200"
                        rows={3}
                        placeholder="Justifique a necessidade de assistência técnica..."
                    />
                </div>
            )}
         </Field>
      ...
      <Section title="5 – DIMENSIONAMENTO DO OBJETO" instruction="O quantitativo deve ser justificado a partir de uma estimativa, que pode levar em conta a série histórica, levantamento atual ou o planejamento para evento futuro, com especificação da metodologia adotada. Na descrição do quantitativo, deve constar a memória de cálculo para estimativa com base na metodologia utilizada, indicando eventuais inconsistências no dimensionamento, como objeto insuficiente ou excessivo.">
        ...
        <Field label="5.2. Descrição do Quantitativo" required>
            <div className="relative">
                <textarea name="descricaoQuantitativo" value={data.descricaoQuantitativo} onChange={handleChange} required className={`${inputClasses} h-24 pr-10`}/>
                <AiAssistant fieldName="Descrição do Quantitativo" onGeneratedText={(text) => setData(prev => ({ ...prev, descricaoQuantitativo: text }))} />
            </div>
        </Field>
        <div className="mt-4">
          ...
          {data.itens.map((item, index) => (
            <div key={item.id} className="p-4 border rounded-md mb-2 bg-white dark:bg-gray-800 dark:border-gray-600">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <Field label="Quantidade" required><input type="number" min="0" value={item.quantidade} onChange={e => handleItemChange(item.id, 'quantidade', parseFloat(e.target.value) || 0)} required className={inputClasses}/></Field>
      ...
      <Section title="6 – ESTIMATIVA DO VALOR DA CONTRATAÇÃO" instruction="Não se trata de uma pesquisa mercadológica propriamente dita, mas sim uma simples estimativa do valor da contratação, a partir de uma pesquisa com os dados disponíveis ao público.">
        <Field label="6.1 - Meios usados na pesquisa">
             <div className="flex flex-col gap-y-2">
                ...
                {data.meiosPesquisaOutro.includes('Outro.') && <input type="text" name="meiosPesquisaOutro" value={data.meiosPesquisaOutro} onChange={handleChange} placeholder="Especificar" className={`${inputClasses} mt-2 ml-6`}/>}
            </div>
        </Field>
